...
float sequence(char* s1, char* s2, float gapOpen, float gapExtend,float zeroShift, ScoringMatrix* matrix)
{
  //Setup
  float maximalScore;
  int s1len = strlen(s1);
  ...
  for(i = 1; i <s1len + 1;i++)
  {
    for(j = 1; j <s2len + 1;j++)
    {
      G[i][j] = V[i-1][j-1] + score(s1[i-1],s2[j-1],matrix) + zeroShift;
...
int main(int argc, char* argv[])
{
  if(argc < 7)
  {
    printf("Use : AlignSequence <scoringmatrix> <sequence1> <sequence2> <gapOpenCost> <gapExtensionCost> <zeroShiftCost>\n");
    return -1;
  }
